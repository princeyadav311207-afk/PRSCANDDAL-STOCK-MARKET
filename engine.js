(function (root) {
  ...
  function validateBet(bet, playersById, options = {}) {
    ...
    // In the browser game, balances are reduced when each bet is locked.
    // Do not compare the bet again with that already-reduced balance at resolution.
    if (!options.betsAlreadyDeducted && bettor.balance < bet.amount) throw new Error("Insufficient balance");
  }
  ...
  function resolveRound(statementId, bets, players, options = {}) {
    ...
    const totalWinningStake = money(winningBets.reduce((sum, bet) => sum + bet.amount, 0));
    ...
    if (winningBets.length > 0) {
      ...
      winningBets.forEach((bet, index) => {
        const isLastWinner = index === winningBets.length - 1;
        const prizeShare = isLastWinner
          ? money(losingPool + carryIn - distributed)
          : money((losingPool + carryIn) * (bet.amount / totalWinningStake));
        distributed = money(distributed + prizeShare);
        bet.payout = money(bet.amount + prizeShare);
        bet.balanceDelta = prizeShare;
      });
      payoutPerWinner = winningBets.length ? money((losingPool + carryIn) / winningBets.length) : 0;

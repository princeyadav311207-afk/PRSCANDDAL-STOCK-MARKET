(function () {
  ...
  function availableBalance(value) {
    return Math.floor(E.money(value));
  }

  function isBrokePlayer(p) {
    return availableBalance(p?.balance) <= 0;
  }

  function nextBettingIndex(fromIndex = state.bettorIndex) {
    let index = fromIndex;
    while (index < state.players.length && isBrokePlayer(state.players[index])) index += 1;
    return index;
  }

  function skippedBettorNames(fromIndex, toIndex) {
    return state.players
      .slice(fromIndex, toIndex)
      .filter((p) => isBrokePlayer(p))
      .map((p) => p.name.trim())
      .filter(Boolean);
  }
  ...
  function startGame() {
    ...
    const players = state.players.map((p) => ({
      ...p,
      chosenStatementIds: playerChoiceIds(p),
      balance: E.money(state.startingBalance),
  ...
  function beginBetting() {
    const nextIndex = nextBettingIndex(0);
    if (nextIndex >= state.players.length) {
      toast("Everyone is out of money, so this round will resolve now.");
      resolveCurrentRound();
      return;
    }
    const skipped = skippedBettorNames(0, nextIndex);
    setState({
      phase: "betting",
      bettorIndex: nextIndex,
      activeReady: false,
      selectedTargetId: "",
      betAmount: 20,
      currentBets: [],
    });
    if (skipped.length) toast(`${skipped.join(", ")} ${skipped.length === 1 ? "is" : "are"} out of money and skipped.`);
  }

  function lockBet() {
    const bettor = state.players[state.bettorIndex];
    if (!bettor) return;
    const available = availableBalance(bettor.balance);
    const requested = Math.round(Number(state.betAmount));
    if (!state.selectedTargetId) return toast("Choose a player to bet on.");
    if (!Number.isFinite(requested) || !Number.isInteger(requested)) return toast("Bet must be a whole number.");
    if (requested < E.BET_MIN || requested > E.BET_MAX) return toast("Bet must be an integer from ₹20 to ₹100.");
    if (available < E.BET_MIN) return toast(`${bettor.name} has no money left and is skipped.`);
    ...
    const nextIndex = nextBettingIndex(state.bettorIndex + 1);
    const skipped = skippedBettorNames(state.bettorIndex + 1, nextIndex);
  ...
  function undoLastBet() {
    ...
    state.bettorIndex = Math.max(0, state.players.findIndex((p) => p.id === last.bettorId));
  ...
  function bettingHtml(current) {
    ...
    const available = availableBalance(bettor.balance);
    const maxBet = Math.min(E.BET_MAX, available);
    const canBet = available >= E.BET_MIN;
      ...
      <section class="grid" style="margin-top:18px">
        <div class="statement-stage" style="min-height:220px">
          <div>
            ...
            <span class="pill">Balance ${currency(available)}</span>
            <span class="pill">${locked} / ${state.players.length} bets locked</span>
          </div>
        </div>
        ${available <= 0 ? `
          <div class="panel"><h2>${escapeHtml(bettor.name)} is out of money</h2><p class="notice">${escapeHtml(bettor.name)} is skipped and cannot bet anymore.</p><button class="btn primary" data-action="skipBrokePlayer">Continue</button></div>` : canBet ? `
          <div class="panel">
          ...
          <div class="panel"><h2>Insufficient balance</h2><p class="notice">${escapeHtml(bettor.name)} has less than ₹20 and cannot place a bet.</p><button class="btn primary" data-action="skipBrokePlayer">Skip Turn</button></div>`}
      </section>`;
  }
  ...
  function resultsHtml(round) {
    ...
    const topPayout = round.bets.reduce((max, bet) => Math.max(max, bet.payout || 0), 0);
    return `
      <section class="grid" style="margin-top:18px">
        ...
        <div class="grid three">
          ...
          <div class="metric"><span>Top Winning Payout</span><strong>${currency(topPayout)}</strong></div>
        </div>
  ...
  function handle(action, el) {
    const actions = {
      ...
      lockBet,
      skipBrokePlayer: () => {
        const next = nextBettingIndex(state.bettorIndex + 1);
        if (next >= state.players.length) resolveCurrentRound();
        else setState({ bettorIndex: next, activeReady: false, selectedTargetId: "", betAmount: 20 });
      },
      ...
      restartGame: () => {
        state.players = state.players.map((p) => ({ ...p, chosenStatementIds: playerChoiceIds(p), balance: E.money(state.startingBalance), stats: E.defaultStats() }));
        Object.assign(state, {
          screen: "game",
          phase: "reveal",

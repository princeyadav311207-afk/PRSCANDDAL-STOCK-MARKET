...
test("multiple winners receive their bet back plus a proportional pool share", () => {
  const result = E.resolveRound("7", [
  ...
  assert.equal(result.bets.find((bet) => bet.id === "b1").payout, 80);
  assert.equal(result.bets.find((bet) => bet.id === "b2").payout, 160);
  assert.equal(result.players.find((p) => p.id === "aman").balance, 1060);
  assert.equal(result.players.find((p) => p.id === "rohan").balance, 1120);
});
...
test("already deducted bets resolve when a final bettor uses their remaining balance", () => {
  const result = E.resolveRound("7", [
    { id: "b1", bettorId: "rahul", targetId: "aman", amount: 100 },
    { id: "b2", bettorId: "aman", targetId: "rahul", amount: 100 },
    { id: "b3", bettorId: "priya", targetId: "rohan", amount: 100 },
  ], players({
    rahul: { balance: 0 },
    aman: { balance: 0 },
    priya: { balance: 0 },
  ...
  assert.equal(result.players.find((p) => p.id === "aman").balance, 300);
});
